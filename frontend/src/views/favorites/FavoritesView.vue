  ...
  <div class="space-y-6">
    ...
    <div class="glass-panel p-5 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex flex-wrap items-center gap-3">
        ...
        />
        <el-button type="primary" :icon="Search" @click="loadData(1)" id="btn-favorites-search">
          搜索
        </el-button>
      ...
      <div class="text-xs text-slate-400">
        已收藏 <span class="font-semibold text-amber-300">{{ total }}</span> 部精彩视频
      </div>
    </div>
        ...
        <el-table-column label="时长 / 进度" width="180">
          <template #default="{ row }">
            <div class="text-xs text-slate-300 font-mono py-1">
              ...
              <div v-if="row.progress_seconds > 0" class="text-amber-400/90 mt-0.5">
                进度: {{ formatDuration(row.progress_seconds) }}
              </div>
            </div>
          </template>
        </el-table-column>
...
async function loadData(targetPage = page.value) {
  ...
  try {
    const res = await getFavoritesListApi({
      page: page.value,
...
async function handleDelete(row: FavoriteItem) {
  try {
    await ElMessageBox.confirm(`确定要取消收藏《${row.title}》吗？`, '取消收藏确认', {
      confirmButtonText: '确定取消',
      cancelButtonText: '暂不',
      type: 'warning',
    })
    await deleteFavoriteApi(row.id)
